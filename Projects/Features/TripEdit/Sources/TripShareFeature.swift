    ...
    public var body: some ReducerOf<Self> {
        ...
        Reduce { state, action in
            ...
            case let .shareResponse(.failure(error)):
                FlowneyLog.error("공유 정보 조회 실패: \(error)", category: .share)
                state.isLoading = false
                state.errorMessage = error.localizedDescription
                return .none

            case .generateButtonTapped:
                FlowneyLog.debug("공유 링크 생성 시작 trip=\(state.tripId)", category: .share)
                state.isSaving = true
            ...
            case let .upsertResponse(.success(share)):
                FlowneyLog.debug("공유 링크 생성 성공 token=\(share.token)", category: .share)
            ...
            case let .upsertResponse(.failure(error)):
                FlowneyLog.error("공유 링크 생성 실패: \(error)", category: .share)
                state.isSaving = false
                state.errorMessage = error.localizedDescription
                return .none

            case .revokeButtonTapped:
                FlowneyLog.debug("공유 링크 해제 시작 trip=\(state.tripId)", category: .share)
                state.isSaving = true
                state.errorMessage = nil
                let tripID = state.tripId
                return .run { send in
            ...
            case let .revokeResponse(.failure(error)):
                FlowneyLog.error("공유 링크 해제 실패: \(error)", category: .share)
                state.isSaving = false
                state.errorMessage = error.localizedDescription
                return .none
